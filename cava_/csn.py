...
def makeProteinString(variant, transcript, reference, prot, mutprot, coord1):
	...
	if prot[0] == 'X' and mutprot[0] != 'X':
		...
		if nextstop != -1:
			return '_p.X' + str(leftindex) + changeTo3letters(mutprot[0]) + 'extX' + str(nextstop), (
			str(leftindex), 'X', mutprot[0])
		else:
	...
	if len(prot) == 0:
		...
		return '_p.' + changeTo3letters(protcopy[leftindex - 2]) + str(leftindex - 1) + '_' + changeTo3letters(
			protcopy[rightindex]) + str(rightindex + 1) + 'ins' + changeTo3letters(mutprot), (
		       str(leftindex - 1) + '-' + str(rightindex + 1), '-', mutprot)
