...
def getClassAnnotation(variant, transcript, protein, mutprotein, loc, ssrange, reference, exonseqs, utr5, utr5mut):
    ...
    if variant.is_in_frame and protL > 0 and mutprotL > 0 and (len(mutprotein) >= 1 or len(protein) >= 1) and (
            variant.is_deletion or variant.is_insertion or variant.is_complex):
        if potSS:
            return 'EE'
        return 'IF'
    ...
    if chkutr == 'UTR5':  # Even if a variant covers more than UTR5, UTR5 has priority. .. also includes UTR5-Ex* .. or Ex*-UTR3
        ...
        if 'Ex' in loc and (variant.is_deletion or variant.is_complex):
            return 'SL'
        return '3PU'
    elif loc.startswith('<') and 'Ex' in loc and (
...
def getSequenceOntologyAnnotation(variant, transcript, protein, mutprotein, loc):
    ...
    if where.find('-In') > 0: return 'splice_acceptor_variant'
    if where.find('-Ex') > 0: return 'splice_donor_variant'
    ...
    # At this point there is no splice junctipn of IM/SL in the UTR .. so return if in UTR.
    if chkutr == 'UTR5':
